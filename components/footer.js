...
export default function Footer() {
    ...
    const langObj = footerLang[locale];
            ...
            <section className="pt-10 md:p-5 lg:p-24 lg:pt-16 lg:pb-16 md:pb-7">
                <section className="w-full h-2/6 pb-7 px-6 md:pb-16 lg:px-[5%] border-b border-b-gray-200 flex flex-row flex-wrap justify-between items-start">
                    <div className="w-full lg:w-1/2 flex justify-around items-start">
                        <span className='w-1/2 md:w-48pr mx-auto lg:mx-0 mb-7 flex flex-col justify-center text-center'>
                            ...
                            <h5 className="my-3 text-sm md:text-base font_urbanist_bold">{langObj.securePay.text}</h5>
                            <p className="w-4/5 mx-auto text-xs lg:text-sm">{langObj.securePay.description}</p>
                        </span>
                        <span className='w-1/2 md:w-48pr mx-auto lg:mx-0 mb-7 flex flex-col justify-center text-center'>
                            <i className="fa-solid fa-wallet text-black mb-5 text-xl"></i>
                            <h5 className="my-3 text-sm md:text-base font_urbanist_bold">{langObj.trusPay.text}</h5>
                            <p className="w-4/5 mx-auto text-xs lg:text-sm">{langObj.trusPay.description}</p>
                        </span>
                    </div>
                    ...
                    <div className="w-full lg:w-1/2 flex justify-around items-start">
                        <span className='w-1/2 md:w-48pr mx-auto lg:mx-0 mb-7 flex flex-col justify-center items-center text-center' >
                            ...
                            <h5 className="my-3 text-sm md:text-base font_urbanist_bold">{langObj.helpCenter.text}</h5>
                            <p className="w-4/5 mx-auto text-xs lg:text-sm">{langObj.helpCenter.description}</p>
                        </span>
                        <span className='w-1/2 md:w-48pr mx-auto lg:mx-0 mb-7 flex flex-col justify-center items-center text-center' >
                            <Image className='mb-5' src={truck} alt="delivery vehicle" />
                            <h5 className="my-3 text-sm md:text-base font_urbanist_bold">{langObj.expressShipping.text}</h5>
                            <p className="w-4/5 mx-auto text-xs lg:text-sm">{langObj.expressShipping.description}</p>
                        </span>
                    </div>
                </section>
                ...
                <section className="w-full mt-10 px-6 lg:px-0 font_urbanist_light text-sm grid grid-cols-2 md:flex md:flex-wrap justify-between items-start md:items-start">
                    ...
                    <nav className="list-none pb-7 md:pb-3 gap-y-2 lg:gap-y-4 flex flex-col items-start">
                        ...
                        <li>
                            <Link href='/faq'>{langObj.servicesMenu.item1}</Link>
                        </li>
                        <li>
                            <Link href='/contact'>{langObj.servicesMenu.item2}</Link>
                        </li>
                        <li>
                            <Link href='/customerservices/sizeguide'>{langObj.servicesMenu.item3}</Link>
                        </li>
                        <li>
                            <Link href='/customerservices/cfproducts'>{langObj.servicesMenu.item4}</Link>
                        </li>
                        <li>
                            <Link href='/faq'>{langObj.servicesMenu.item5}</Link>
                        </li>
                    </nav>
                    <nav className="list-none pb-7 md:pb-3 space-y-2 lg:space-y-4 flex flex-col items-start">
                        <h3 className="text-sm lg:text-base font_urbanist_bold">{langObj.exploreMenu.heading}</h3>
                        <li>
                        ...
                        {user && user.email ?
                            <li>
                                <Link href='/user/personalinfo'>{langObj.exploreMenu.item4}</Link>
                            </li> : null}
                    </nav>
                    <nav className="list-none pb-7 md:pb-3 space-y-2 lg:space-y-4 flex flex-col items-start">
                        <h3 className="text-sm lg:text-base font_urbanist_bold">{langObj.ordersMenu.heading}</h3>
                        <li>
                            <Link href='/user/orders/processing'>{langObj.ordersMenu.item1}</Link>
                        </li>
                        <li>
                            <Link href='/trackorder'>{langObj.ordersMenu.item2}</Link>
                        </li>
                        <li>
                            <Link href='/customerservices/delivery'>{langObj.ordersMenu.item3}</Link>
                        </li>
                        <li>
                            <Link href='/customerservices/returns&refund'>{langObj.ordersMenu.item4}</Link>
                        </li>
                    </nav>
                    <nav className="list-none pb-7 md:pb-3 space-y-2 lg:space-y-4 flex flex-col items-start">
                        <h3 className="text-sm lg:text-base font_urbanist_bold">{langObj.aboutMenu.heading}</h3>
                        <li>
                            <Link href='/policies/terms&conditions'>{langObj.aboutMenu.item1}</Link>
                        </li>
                        <li>
                            <Link href='/policies/privacypolicy'>{langObj.aboutMenu.item2}</Link>
                        </li>
                        <li>
                            <Link href='/policies/cookiespolicy'>{langObj.aboutMenu.item3}</Link>
                        </li>
                        <li>
                            <Link href='/customerservices/companyinfo'>{langObj.aboutMenu.item4}</Link>
                        </li>
                    </nav>
                    <nav className="w-full md:w-1/5 xl:w-auto col-span-2 flex flex-col justify-start items-start md:items-end gap-y-3 md:gap-y-5">
                        <h3 className="hidden lg:block text-sm lg:text-base font_urbanist_bold"><Link href="/contact">{langObj.connect}</Link></h3>
                        ...
                        <div className="lg:mt-6 flex flex-col items-end">
                            <h3 className="hidden lg:block text-sm font_copper tracking-1 uppercase">{langObj.paymentMethods}</h3>
