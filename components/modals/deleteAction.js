...
import { deleteModal as delModalLang } from "@/locales/index";

export default function DeleteAction(props) {
    const { locale } = useLanguage();
    const [actionValue, setActionValue] = useState('');
    const langObj = delModalLang[locale];
                ...
                <><button onClick={() => { props.setDeleteModal(null); setActionValue('') }} name="deleteModal" className="material-symbols-rounded text-3xl absolute right-5 top-5 cursor-pointer hover:rotate-180 transition-all duration-1000">close</button>
                    <section className="w-full h-full p-6">
                        <div className="w-full space-y-3">
                            <h2 className="text-black font_urbanist_bold text-base md:text-lg lg:text-xl">{props.heading || langObj.delete}</h2>
                            <p className='text-sm text-red-500'>{props.msg}</p>
                            <p className='text-sm '>{langObj.deleteMsg.pt1} <span className='text-red-600' >"delete"</span> {langObj.deleteMsg.pt2}</p>
                        </div>
                        <div className="relative w-full mt-5 data_field flex items-center border-b border-b-gray-200 focus:border-pinky hover:border-pinky transition py-2 mb-6">
                        ...
                        <div className="w-full mt-7 flex gap-x-2">
                            <Button my="0" onClick={() => { props.setDeleteModal(null); setActionValue('') }} bg="bg-gray-100" text="black" classes="w-1/2" font='font_urbanist_medium'>{langObj.cancelBtn}</Button>
                            <Button my="0" disabled={actionValue !== "delete"} classes="w-1/2" onClick={() => {
                                props.onTakeAction();
                                setActionValue('');
                                props.setDeleteModal(null)
                            }}>{langObj.delete}</Button>
                        </div>
                    </section></>
            }
        </div>
