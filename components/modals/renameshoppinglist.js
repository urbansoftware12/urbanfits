...
export default function RenameShopListModal({ show, setRenameListModal, list, langObj }) {
        ...
        <div className={`${show ? null : "scale-0"} relative w-11/12 md:w-3/5 lg:w-[33rem] text-sm flex flex-col lg:flex-row bg-white rounded-2xl md:rounded-3xl overflow-hidden transition-all duration-500`}>
            ...
            <section className="w-full h-full p-6">
                <div className="w-full space-y-5">
                    <h2 className="text-black md:text-lg lg:text-xl font_urbanist_bold">{langObj.rename} '{list.name}'</h2>
                    <p className='text-xs md:text-sm'>{langObj.renameTip}</p>
                </div>
                ...
                <Button disabled={!listname} onClick={async () => { await renameShoppingList(list._id, listname); setRenameListModal(false) }} my="my-2" classes="w-full text-sm md:text-base text-center">{langObj.renameBtn}</Button>
            </section>
        </div>
    </div>
}