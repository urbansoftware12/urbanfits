...
export default function CreateShopListModal(props) {
    const { createShoppingList } = useShoppingList();
    const [listname, setListname] = useState('')

    return <div className={`${props.show ? null : "opacity-0 pointer-events-none"} w-full h-full font_urbanist fixed inset-0 z-[100] bg-gray-800/40 backdrop-blur flex justify-center items-center transition-all duration-500`}>
        <div className={`${props.show ? null : "scale-0"} relative w-11/12 md:w-3/5 lg:w-[33rem] text-sm flex flex-col lg:flex-row bg-white rounded-2xl md:rounded-3xl overflow-hidden transition-all duration-500`}>
            <button onClick={() => props.setListModal(false)} className="material-symbols-rounded text-3xl absolute right-5 top-5 cursor-pointer hover:rotate-180 transition-all duration-1000">close</button>
            <section className="w-full h-full p-6">
                <div className="w-full space-y-5">
                    <h2 className="text-black md:text-lg lg:text-xl font_urbanist_bold">Create Shopping List</h2>
                ...
                <Button disabled={!listname} onClick={async() => {await createShoppingList(listname); props.setListModal(false)}} my="my-2" classes="w-full text-sm md:text-base text-center">Create List</Button>
            </section>
        </div>
    </div>
}