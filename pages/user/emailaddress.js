    ...
    if (user && window.matchMedia('(max-width: 760px)').matches) return <>
        ...
        <main className='w-screen h-screen bg-white flex flex-col transition-all duration-500'>
            ...
            <div className="w-full mt-7 px-4 flex flex-col">
                <div className="flex justify-between items-center p-4 rounded-md bg-gray-50 font_urbanist text-sm">
                    ...
                    {user.register_provider !== "urbanfits" ? null : <Link href="/user/change-email">{langObj.change}</Link>}
                </div>
                <p className="mt-2 font_urbanist text-[15px]">{langObj.emailTip}</p>
                <div className="w-full mt-5 flex items-center justify-between font_urbanist_medium text-base">
                    {langObj.emailVerfication}
        ...
        <User loading={loading}>
            <form className="w-full mt-10 font_urbanist text-sm overflow-x-hidden" onReset={handleReset} onSubmit={handleSubmit} >
                <h1 className='mb-10 text-sm lg:text-base font_urbanist_bold'>{langObj.title}</h1>
                <h2 className='text-sm font_urbanist_medium'>{langObj.currentMail}</h2>
                ...
                <h2 className='text-sm font_urbanist_medium' >{langObj.newMail}</h2>
                ...
                <div className='w-full flex justify-between items-center text-sm font_urbanist_medium' ><h2>{langObj.currentPass}</h2> <button type='button' onClick={() => { logOut(); router.push("/auth/resetpassword") }} className='text-xs flex items-center' >Change<i className="material-symbols-outlined text-sm ml-2">edit_square</i></button></div>
                ...
                {otpId ? <><h2 className='mt-10 text-[15px] font_urbanist_bold' >Enter OTP</h2>
                    ...
                    <div className="w-full flex items-center text-xs font_urbanist text-gray-400 gap-x-10">
                        {langObj.gotNoOtp}
                        <button type="submit" className="text-black underline">{langObj.sendOtpBtn}</button>
                    </div>
                </> : null}
                <div className="w-full flex justify-end space-x-4">
                    ...
                    {otpId ? <Button onClick={matchOtpAndUpdateEmail} type="button" loading={loading} disabled={!otp || otp.length < 5} classes="w-full md:w-1/3" font='font_urbanist_medium'>{langObj.cancelBtn}</Button> :
                        <Button loading={loading} type="submit" classes="w-full md:w-1/3" font='font_urbanist_medium'>{langObj.otpBtn}</Button>}
                </div>
            </form>
        </User>
    </>
